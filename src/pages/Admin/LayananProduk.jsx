...
const TambahLayananPage = () => {
  ...
  // Load icons saat modal dibuka - NEW
  useEffect(() => {
    if (showIconModal) {
  ...
  const handleSave = async () => {
    // Validasi
    if (!namaLayanan.trim()) {
      return Swal.fire("Validasi Gagal", "Nama layanan wajib diisi.", "warning");
    }
    if (!deskripsiSingkat.trim()) {
      return Swal.fire("Validasi Gagal", "Deskripsi singkat wajib diisi.", "warning");
    }
    if (!deskripsi.trim()) {
      return Swal.fire("Validasi Gagal", "Deskripsi lengkap wajib diisi.", "warning");
    }
    if (layananDitawarkan.length === 0) {
      return Swal.fire("Validasi Gagal", "Minimal harus ada 1 poin layanan.", "warning");
    }
    if (!foto && !previewFoto) {
      return Swal.fire("Validasi Gagal", "Foto layanan wajib diupload.", "warning");
    }
    if (!selectedIcon) {
      return Swal.fire("Validasi Gagal", "Icon layanan wajib dipilih.", "warning");
    }
    ...
    try {
      ...
      formData.append("sub_services_data", JSON.stringify(layananDitawarkan));
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FB] font-['Poppins'] pb-10">
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-xl aspect-video flex items-center justify-center bg-gray-50 relative overflow-hidden group hover:border-green-500 transition-colors">
                {previewFoto ? (
                  <>
                    ...
                    <button onClick={removeFoto} className="absolute top-2 right-2 bg-red-500 text-white p-1 rounded-full shadow hover:scale-110 transition">
                      <X size={14} />
                    </button>
                  </>
                ) : (
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-xl aspect-video flex items-center justify-center bg-gray-50 relative overflow-hidden group hover:border-green-500 transition-colors">
                {selectedIcon ? (
                  <>
                    ...
                    <button onClick={removeIcon} className="absolute top-2 right-2 bg-red-500 text-white p-1 rounded-full shadow hover:scale-110 transition">
                      <X size={14} />
                    </button>
                  </>
                ) : (
                ...
                {layananDitawarkan.length === 0 ? (
                  ...
                  layananDitawarkan.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-center bg-white px-4 py-2 rounded border shadow-sm">
                      ...
                      <button onClick={() => hapusLayananItem(idx)} className="text-red-500 hover:text-red-800">
                        <Trash2 size={24} />
                      </button>
                    </div>
                  ))
      ...
      {/* Font Awesome Icon Modal - UPDATED */}
      {showIconModal && (
          ...
          <div className="bg-white rounded-xl p-4 shadow-lg relative w-[90%] h-[80%] max-w-lg flex flex-col">
            ...
            <div className="relative flex-1">
              <Cropper
                ...
                style={{ containerStyle: { width: "100%", height: "100%", backgroundColor: "#f3f3f3" } }}
              />
            </div>
            <input type="range" min={1} max={3} step={0.1} value={zoom} onChange={(e) => setZoom(e.target.value)} className="mt-3" />
            <div className="flex justify-end gap-3 mt-4">
              <button className="px-4 py-2 bg-gray-300 rounded" onClick={() => setCropModalOpen(false)}>
                Batal
              </button>
              <button className="px-4 py-2 bg-green-600 text-white rounded" onClick={selesaiCrop}>
                Selesai
              </button>
            </div>
          </div>
