...
const Sidebar = ({ isOpen, setIsOpen }) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const normalizedMenus = rawMenus.map((m) => {
            ...
            // === CUSTOM URL REWRITE ===
            if (fixedUrl === "/Admin/Users")
              fixedUrl = "/Admin/AdminManagement";
            if (fixedUrl === "/Admin/Proyek/Produk") fixedUrl = "/Admin/Produk";
          ...
          const structuredMenu = parents.map((parent) => {
            // === FILTER DUPLIKAT DARI DB ===
            // Ambil anak menu dari DB, TAPI JANGAN ambil yang namanya "+ Tambah Layanan"
            // Karena kita akan membuatnya sendiri secara manual di bawah
            let myChildren = children
              .filter((child) => child.parent_id === parent.id)
              .filter((child) =>
                parent.nama_menu === "Perusahaan"
                  ? false // BUANG semua submenu perusahaan dari DB
                  : !child.nama_menu.toLowerCase().includes("tambah layanan")
              )
              .map((child) => ({
            ...
            if (parent.nama_menu === "Layanan" || parent.url === "#Layanan") {
              myChildren = [
                ...myChildren, // Submenu dari DB (Bersih tanpa duplikat)
                ...servicesMenu, // Service dari API
                {
                  name: "Tambah Layanan",
  ...
  const renderMenuItem = (item, index) => {
    ...
    const isActiveParent =
      ...
      (hasSubItems &&
        item.subItems.some((sub) => router.pathname === sub.path));
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center gap-3 px-6 py-2.5 rounded-lg text-sm font-['Poppins'] transition-colors 
                        ${
                          isSpecialBtn
                            ? // Style Tombol Tambah Layanan (Hijau, Sejajar, Ikon Plus)
                              "mt-2 text-white bg-green-500 hover:bg-green-600 shadow-md justify-center font-semibold"
                            : isSubActive
                      ...
                      `}
                    >
                      {/* Ikon Bulat untuk menu biasa */}
                      {!isSpecialBtn && (
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${
                            isSubActive ? "bg-[#39B54A]" : "bg-gray-300"
                          }`}
                        ></span>
                      )}

                      {/* Ikon Plus untuk tombol spesial */}
                      {isSpecialBtn && (
      ...
      <div className="hidden lg:flex h-screen w-[280px] bg-white shadow-2xl flex-col sticky top-0 z-50">
        ...
        <div className="p-6 mt-auto border-t border-gray-100 bg-white">
          <button
            ...
            className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl font-['Poppins'] font-medium text-white bg-[#FF3B3B] hover:bg-[#e63535] transition-all shadow-lg shadow-red-100"
          >
            <span>Log out</span>
          </button>
        </div>
        ...
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative w-[280px] bg-white shadow-2xl flex flex-col h-full">
            ...
            <div className="p-6 mt-auto border-t">
              <button
                onClick={handleLogout}
                className="flex items-center justify-center gap-2 w-full py-3 rounded-xl font-['Poppins'] font-medium text-white bg-[#FF3B3B] hover:bg-[#e63535]"
              >
                <span>Log out</span>
              </button>
            </div>
