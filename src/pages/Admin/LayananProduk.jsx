...
const TambahLayananPage = () => {
  ...
  // --- LOGIKA SIMPAN DATA (PERBAIKAN & VALIDASI) ---
  const handleSave = async () => {
    ...
    if (!deskripsiSingkat.trim()) {
      return Swal.fire("Validasi Gagal", "Deskripsi singkat (Ringkasan) wajib diisi.", "warning");
    }
    if (!deskripsi.trim()) {
      return Swal.fire("Validasi Gagal", "Deskripsi lengkap wajib diisi.", "warning");
    }
    if (layananDitawarkan.length === 0) {
      return Swal.fire("Validasi Gagal", "Minimal harus ada 1 poin layanan yang ditawarkan.", "warning");
    }
    if (!foto && !previewFoto) {
    ...
    try {
      ...
      if (res.ok) {
        Swal.fire({
            title: "Berhasil!",
            text: "Layanan berhasil disimpan dengan struktur yang benar.",
            icon: "success",
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-6">
            ...
            <div>
             ...
             focus:border-green-500 
             focus:ring-1 focus:ring-green-500
             rounded-lg px-4 py-3 text-gray-800 transition-all" placeholder="" />
            </div>
