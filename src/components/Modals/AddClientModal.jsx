    ...
    try {
      ...
      const res = await fetch('/api/clients', {
        ...
        body: JSON.stringify({
          ...
          userId: user?.id // 3. KIRIM USER ID KE BACKEND
        }),
      });
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            <div className="relative w-full h-40 bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl overflow-hidden flex items-center justify-center mb-3 group hover:border-green-500 transition-colors">
              {previewUrl ? (
                <div className="relative w-full h-full p-4">
                   ...
                   <button type="button" onClick={() => { setLogoFile(null); setPreviewUrl(null); }} className="absolute top-2 right-2 bg-white p-1 rounded-full shadow-md hover:bg-red-50 text-red-500"><X size={16} /></button>
                </div>
              ) : (
          ...
          <div>
            ...
            <input type="text" value={clientName} onChange={(e) => setClientName(e.target.value)} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-green-500 text-gray-800 placeholder-gray-400" placeholder="Nama Perusahaan Client" />
          </div>

          <div className="flex justify-end gap-3 pt-4">
            <button type="button" onClick={handleClose} className="px-6 py-2.5 rounded-lg border border-gray-300 text-gray-700 font-medium hover:bg-gray-50 transition-colors" disabled={isSubmitting}>Batal</button>
            <button type="submit" disabled={isSubmitting} className="px-6 py-2.5 rounded-lg bg-[#2D2D39] text-white font-medium hover:bg-black transition-colors shadow-lg disabled:opacity-50 flex items-center gap-2">
